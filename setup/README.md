# Setup — Installation des ressources

Guides d'installation et de configuration des outils utilisés dans la formation. À suivre avant les ateliers Phase 2 côté client (les prérequis d'entreprise — licences, tenant — sont dans [docs/01-prerequis-activation](../docs/01-prerequis-activation/)).

| # | Guide | Outil | Utilisé par | Statut |
|---|---|---|---|---|
| 01 | [Power BI Desktop](01-powerbi-desktop.md) | Desktop : aperçus PBIP + Prep data for AI, relations, Tabular Editor | Modules 01, 02, 04, 05 | ✅ |
| 02 | VS Code + Copilot + Power BI MCP Server | VS Code, extension MCP, semantic-link | Module 04 (MCP Data Modelling) | À venir |
| 03 | fab CLI + Fabric Skills | Fabric CLI, espaces de travail Skills | Module 05 (Fabric Skills Authoring) | À venir |
| 04 | Capacité Fabric & gouvernance | Portail admin, Capacity Metrics App | Module 06 (Gouvernance & CUs) | À venir |

## Vérification rapide (tous modules)

- [ ] Power BI Desktop à jour avec les aperçus activés (guide 01)
- [ ] Compte connecté avec accès à une capacité Fabric (F64+ ou P1) dans une région supportée
- [ ] Espace de travail Copilot-enabled disponible pour les tests
